...
function fetchPNRdetails() {
    ...
    chrome.runtime.sendMessage({
        method: 'GET',
        action: 'xhttp',
        url: 'https://api.railwayapi.com/v2/pnr-status/pnr/' + pnr_no + '/apikey/' + apikey + '/',
        data: ''
        ...
        if (resp.response_code == '200') {
        ...
        } else if (resp.response_code == '220') {
            $result_block.html('<h3 id="loading_span" class="loading_span">Flushed PNR. <br> <span>Please try another PNR number.</span></h3>');
        } else if (resp.response_code == '221') {
            $result_block.html('<h3 id="loading_span" class="loading_span">Invalid PNR. <br> <span>Please try another PNR number.</span></h3>');
        } else if (resp.response_code == '304') {
            $result_block.html('<h3 id="loading_span" class="loading_span">Data could not be fetched.  <br> <span>No Data available.</span></h3>');
            ...
            $result_block.html('<h3 id="loading_span" class="loading_span">Data could not be fetched <br> <span> Request could not go through.</span></h3>');
        ...
        } else if (resp.response_code == '504') {
            $result_block.html('<h3 id="loading_span" class="loading_span">Argument error. <br> <span>Please verify all details.</span></h3>');
        } else if (resp.response_code == '704') {
            $result_block.html('<h3 id="loading_span" class="loading_span">Unauthorized user query. <br><span>User account expired/exhausted or unregistered.</span></h3>');
        } else {
